...
### Export
- Export all recipes in standardized format (e.g., schema.org Recipe) — implemented:
  `GET /api/export?format=schema-org` downloads a JSON array of schema.org `Recipe` objects
  (latest version of each recipe; archived recipes are included, flagged via
  `creativeWorkStatus`); ingredient `{ref}` tokens in steps are resolved to plain text
  server-side.
- If standardized format loses custom data, also support proprietary format export —
  implemented: `GET /api/export?format=full` downloads a versioned envelope
  (`{ formatVersion, exportedAt, data }`) with every recipe version, notes, per-ingredient
  catalog IDs, meal plans, and the user's private catalog/substitution/localization data, plus
  a media manifest (paths only — media bytes stay in the data volume). Both are reachable from
  the recipe list's Export menu.
