...
- [x] [18 — Canonical ingredient units](18-unit-normalization.md) (M) — `src/constants/units.ts` owns the canonical vocabulary (`tsp`/`tbsp`/`cup`/`fl oz`/`pt`/`qt`/`gal`/…) + synonym table (long/plural/period/case forms) with `system`/`kind` metadata populated for plan 27; `normalizeUnit()` resolves the case-sensitive `T`→tbsp / `t`→tsp shorthand before the lowercase pass and passes unknown units through trimmed+lowercased. Normalization runs at write time via a `.transform()` on the recipe Zod ingredient `unit` (covers create/update) and in `import.service.ts`'s line parser; grocery consolidation keys on `normalizeUnit()` defensively so pre-migration spelling variants still collapse. One-time idempotent `scripts/normalize-units.ts` (`npm run db:normalize-units`) converges existing `Ingredient.unit` + `GroceryItem.unit`. Frontend recipe-form unit input already a `ComboInput`; suggestions realigned to the canonical set. Tests: `units.test.ts` table, import `2 Tablespoons`→`tbsp`, grocery `tbsp`+`tablespoon`+`T` → one line, recipe-create write-time normalization
- [x] [19 — Import hardening](19-import-hardening.md) (M) — fixture corpus (`@graph`/array JSON-LD, `HowToSection` flattening, unicode fractions, no-JSON-LD blog fallback); fixed a pre-existing bug where `parseDuration` always returned null for ISO-8601 durations (unanchored regex matched an empty string); decode HTML entities in JSON-LD strings; import preview flags fields the parser didn't find
...
- [x] [29 — Share: native/email/PDF](29-sharing-text-pdf.md) (S-M) — extracted a shared `recipeToText` formatter from `exportRecipe.ts` (now fed by the .txt download, share sheet, and email body); RecipeDetailPage footer gains **Share…** (`navigator.share`, hidden where unsupported, swallows user-cancel `AbortError`), **Email** (`mailto:` subject + body, truncated past ~1800 chars with a "use Share or Download" notice so the action stays usable), and **Save as PDF** (`window.print()` into the existing print stylesheet; `AppLayout` header now `print:hidden` for a clean page). Ingredient `note` intentionally deferred to plan 23. New `recipeToText`/mailto unit tests + share-wiring RTL tests. Follow-up (issue #34): the printed page was clipped after page 1 by the screen layout's `overflow-x: hidden` on `html, body`, dropping the trailing notes — `@media print` now resets that, notes blocks use `break-inside-avoid`, and the printed ingredient list finally shows plan 23's per-ingredient `note`; `PrintLayout.test.tsx` pins it
- [x] [30 — Shareable links (public token view)](30-share-links.md) (M) — new `RecipeShare` model (uuid id doubles as the unguessable token, nullable `revokedAt`, cascade with user); owner endpoints `GET/POST/DELETE /api/recipes/:id/share` (create returns the existing unrevoked token, revoke is idempotent, all 404 on non-owned via `getRecipeVersions`); public `GET /api/shared/:token` (+ `/media/:mediaId`) mounted before the CSRF/requireAuth gates — payload is the chain's latest version with `personalNotes` and user ids stripped, media served token-scoped (own `sendFile`, the authed `/media` mount is untouched, whole surface behind a 100/15 min per-IP limiter). Frontend `/shared/:token` page outside `ProtectedRoute` reuses `RecipeIngredientList`/`resolveIngredientRefs`/`RecipeNotes`; the plan-29 footer gains **Copy link** (mints a share on demand + copies `${origin}/shared/${token}`) and **Revoke link**. 13-test supertest suite (no-auth round-trip, revoked→404, personalNotes absent, media scoping, latest-version tracking, cross-user isolation) + SharedRecipePage/ExportActions RTL tests
- [x] [31 — Bulk export (schema.org + proprietary)](31-schema-org-export.md) (M)
